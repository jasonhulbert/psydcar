# Psydcar POC - Phase Implementation Plan
...
- Keep the first version intentionally local and simple: sidecar source directories may live anywhere on the local filesystem, while app-managed metadata and indexes live under `~/.psydcar`.
...
- Basic CLI entrypoint named `psydcar`.
...
- `psydcar --help` runs locally.
- Python tests can run even if they only cover config/path helpers.
- The app storage root can be resolved, defaulting to `~/.psydcar`.
...
- Sidecar registry under `~/.psydcar/sidecars/<id>/` for app-managed metadata and indexes.
...
- Safe root-relative path resolution against the sidecar source root, not against `~/.psydcar`.
...
- Creating a sidecar can target a fixture directory outside `~/.psydcar`.
...
- CLI commands:
- `psydcar sidecar create`
  - `psydcar sidecar list`
  - `psydcar sidecar delete`
  - `psydcar rebuild`
  - `psydcar refresh`
  - `psydcar search`
  - `psydcar status`
...
- `psydcar mcp --sidecars frontend,backend` command.
- MCP tools:
  - `search`
  - `read_chunk`
  - `read_file`
