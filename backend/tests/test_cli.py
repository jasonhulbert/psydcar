...
def test_help_runs(capsys):
    ...
    assert "Manage local Psydcar indexes." in output
...
def test_cli_config_and_create_use_runtime_defaults(monkeypatch, tmp_path, capsys):
    monkeypatch.setenv("PSYDCAR_HOME", str(tmp_path / "storage"))
    monkeypatch.setenv("PSYDCAR_MAX_FILE_SIZE_BYTES", "123")
    monkeypatch.setenv("PSYDCAR_IGNORED_DIRS", ".git,custom")
    monkeypatch.setenv("PSYDCAR_EMBEDDING_MODEL", "local/model")
    source_root = tmp_path / "source"
    source_root.mkdir()
