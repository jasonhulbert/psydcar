...
from psydcar.index import connect_index, index_path_for_sidecar, rebuild_sidecar_index
from psydcar.mcp import McpError, PsydcarMcpServer, parse_sidecar_ids
...
def test_mcp_initialize_and_tools_list(tmp_path):
    ...
    server = PsydcarMcpServer(registry, ["docs"])

    initialize = server.handle_message(
        {"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {}}
    )
    assert initialize is not None
    assert initialize["result"]["serverInfo"]["name"] == "psydcar"
...
def test_mcp_search_labels_multi_sidecar_results(tmp_path):
    ...
    server = PsydcarMcpServer(registry, ["frontend", "backend"])
